...
async function init() {
  ...
  if (savedRaw) {
    try {
      seriesData = JSON.parse(savedRaw);
    } catch {
      seriesData = [];
    }
  }
...
async function refreshFromTMDB() {
  const needs = seriesData.filter(
    (x) => x.title && (!x._tmdbId || !x._overview || !x._genreIds?.length),
  );
  ...
  for (let i = 0; i < needs.length; i += BATCH) {
    ...
    const results = await Promise.all(
      batch.map((x) => fetchTMDBData(x.title, x.year, x.isMovie)),
    );
    ...
    results.forEach((d, j) => {
      ...
      if (d.overview?.trim()) {
        item._overview = d.overview;
        changed = true;
      }
      if (d.genreIds?.length) {
        item._genreIds = d.genreIds;
        changed = true;
      }
    });
...
function assignCategories() {
  seriesData.forEach((item) => {
    ...
    if (item.isMovie || p.isMovie) {
      item._category = "Movies";
    } else if (p.finished) {
      item._category = "Finished";
    } else if (p.season && p.season > 1) {
      item._category = "Ongoing Series";
    } else {
      item._category = "Watching";
    }

    // Add primary genre as secondary category if available
    if (genres.length) {
      item._primaryGenre = genres[0];
    }
  });
}
...
function populateCategories() {
  ...
  const cats = new Set(["Movies", "Finished", "Ongoing Series", "Watching"]);

  // Add unique genres from all series
  seriesData.forEach((item) => {
    if (item.genres && Array.isArray(item.genres)) {
      item.genres.forEach((g) => cats.add(g));
    }
    if (item._primaryGenre) cats.add(item._primaryGenre);
  });
...
function filterData() {
  ...
  let result = seriesData.filter((item) => {
    ...
    if (cat) {
      catMatch =
        item._category === cat ||
        (item.genres && item.genres.includes(cat)) ||
        item._primaryGenre === cat;
    }
...
function renderHeader() {
  ...
  if (isLoggedIn) {
    c.appendChild(
      mkBtn("btn-ghost btn", ICONS.import + " Import", () =>
    ...
    c.appendChild(mkBtn("btn-ghost btn", ICONS.export + " Export", exportFile));
    c.appendChild(
      mkBtn("btn-accent btn", ICONS.lock + " Login", showLoginModal),
    );
  }
}
...
function renderGrid(list = seriesData) {
  ...
  list.forEach((item) => {
    ...
    const descHtml = item._overview
      ? `<p class="card-desc">${esc(item._overview)}</p>`
      : "";
...
function updateProgress() {
  const hide =
    document.getElementById("mMovie").checked ||
    document.getElementById("mFinished").checked;
...
function renderStars() {
  document
    .querySelectorAll(".star-btn")
    .forEach((b, i) => b.classList.toggle("on", i < currentRating));
  document.getElementById("ratingLabel").textContent = currentRating
    ? currentRating + " / 5"
    : "Not rated";
}
...
function saveModal(index) {
  ...
  if (index >= 0 && seriesData[index].title !== title) {
    ...
    delete newItem._overview;
    delete newItem._genreIds;
  }
